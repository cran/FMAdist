...
packages<-c("fitdistrplus","STAR","EnvStats","extraDistr","MASS","quadprog")
...
MLE<-function(dist,data){
  ...
  if (dist=='normal'){
    ...
    MLE_invgauss<-STAR::invgaussMLE(data)
    theta=MLE_invgauss$estimate #c(mu,sigma2)
    ...
    MLE_loglogis<-STAR::llogisMLE(data)
    theta=MLE_loglogis$estimate #c(location,scale)
...
CDF<-function(dist,x,theta){
  ...
  switch(dist,
         ...
         'inverse gaussian'=STAR::pinvgauss(x,mu=theta[1],sigma2=theta[2]),
         ...
         'loglogistic'=STAR::pllogis(x, location = theta[1], scale = theta[2]),
...
IT<-function(dist,U,theta){
  ...
  switch(dist,
         ...
         'inverse gaussian'=STAR::qinvgauss(U,mu=theta[1],sigma2=theta[2]),
         ...
         'loglogistic'=STAR::qllogis(U, location = theta[1], scale = theta[2]),
